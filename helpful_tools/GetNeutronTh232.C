...
void GetNeutronTh232(const char* ext = ".root"){

  const char* inDir = "/Volumes/GoogleDrive/My Drive/GraduateWork/TESSERACT/Tesseract_sim_output/geom2_helium/NeutronTh232/";
  ...
  for (Int_t i = 0; i < n; i++){
      ...
      MacroInfo m=GetMacroInfo(file, name.substr(100,20));
      //MacroInfo m=GetMacroInfo(file, "Flux_virtual_");
      cout<<name.substr(100,20)<<"   "<<m.SurfaceArea<<endl;
  ...
  TFile* f = TFile::Open("/Volumes/GoogleDrive/My Drive/GraduateWork/TESSERACT/Tesseract_sim_output/geom2_helium/ProcessedFiles/NeutronTh232_processed.root");
  ...
  double specific_activity_Th232=1.0049688079902439e-3;// perSec per m^2 due to 1 Bq/kg from Suerfu
  double activity_Th232_SURF=13;//Bq/kg
  double Particle_per_second= specific_activity_Th232*activity_Th232_SURF*Surface_Area; //perSec
  ...
  leg4->AddEntry(H_Dep, Form("Th232"));
  ...
  TFile *myfile = new TFile("NeutronTh232.root", "RECREATE");
  H_Dep->Write();
  myfile->Close();
