...
void GetGammaU238(const char* ext = ".root"){

  const char* inDir = "/Volumes/GoogleDrive/My Drive/GraduateWork/TESSERACT/Tesseract_sim_output/geom2_helium/GammaU238/";
  ...
  for (Int_t i = 0; i < n; i++){
      ...
      MacroInfo m=GetMacroInfo(file, name.substr(97,18)); //Flux_virtual_
      //MacroInfo m=GetMacroInfo(file, "Flux_virtual_");
      cout<<name.substr(97,18)<<"   "<<m.SurfaceArea<<endl;
  ...
  TFile* f = TFile::Open("/Volumes/GoogleDrive/My Drive/GraduateWork/TESSERACT/Tesseract_sim_output/geom2_helium/ProcessedFiles/GammaU238_processed.root");
  ...
  double specific_activity_U238=235.31936534399997;// perSec per m^2 due to 1 Bq/kg from Suerfu
  double activity_U238_SURF=29;//Bq/kg
  double Particle_per_second= specific_activity_U238*activity_U238_SURF*Surface_Area; //perSec
  ...
  TFile *myfile = new TFile("GammaU238.root", "RECREATE");
  H_Dep->Write();
  myfile->Close();
